...
public class CodigoMorseController {
    ...
    public String tratutorMorse(String textoDoTratutor){
        StringBuilder translatedText = new StringBuilder();
        for (Character letter: textoDoTratutor.toCharArray()){
    ...
    public void playSound(String[] morseMessage) throws LineUnavailableException, InterruptedException {
        ...
        int slahDuration = 2 * dashDuration;

        for(String pattern: morseMessage){
            for (char c: pattern.toCharArray()){
                if (c == '.'){
                    playBeet(sourceDataLine, dotDuration);
                    Thread.sleep(dotDuration);
                }else if (c == '-'){
                    playBeet(sourceDataLine, dashDuration);
                    Thread.sleep(dotDuration);
                }else if (c == '/'){
                    Thread.sleep(slahDuration);
                }
            }
    ...
    private void playBeet(SourceDataLine line, int duration){
